...
2026-09-22 — добавлена самостоятельная сущность `Reminder` (заголовок +
дата + необязательное время, свой CRUD, без ручного порядка): видна
свёрнуто на Today через карточку-переключатель с прогрессом целей
("13.02. Reminder Entity"), и полностью — на Calendar, который стал её
"домашней страницей" (сетка показывает плашки напоминаний в ячейке дня,
панель дня получила отдельную секцию "Напоминания" со своим CRUD); заодно
клик по строке цели на Today теперь открывает её форму редактирования на
странице Goals через deep link `?edit=<id>`. Не путать с уже существующим
"13.01. Due Date Reminders" — тем toast-механизмом, который смотрит на
`Task.dueDate`/`Goal.targetDate` и своих данных не хранит).
