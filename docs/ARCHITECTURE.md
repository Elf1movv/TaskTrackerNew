...
**План исправления и полный список (включая фронтенд, логирование,
Docker)** — `docs/ROADMAP.md`. Ниже — только краткая сводка того, что
касается непосредственно архитектуры бэкенда.
