...
**`FEEDBACK_EMAIL_TO`** (2026-09-19, временная бета-фича — виджет
обратной связи, см. `LEARNING.md`) — необязательная, отдельная от
`RESEND_API_KEY`: адрес получателя отзывов ещё не создан, поэтому пока
она пустая, а отзывы просто копятся в таблице `Feedback` и логе
контейнера. Как только появится инбокс для отзывов — прописать сюда,
письма начнут уходить сами, без передеплоя кода.
