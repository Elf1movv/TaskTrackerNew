...
**Обновлено 2026-09-20** — раньше здесь было устаревшее описание
(единый `PUT` с целым списком). Сейчас гранулярный REST, одна операция
— один запрос, на каждую сущность (задачи/цели/привычки/категории):
- `GET /api/tasks` — получить список (только свои записи, см. 3.4-а
  про авторизацию)
- `POST /api/tasks` — создать одну запись
- `PATCH /api/tasks/:id` — частично обновить одну запись; тело
  `{ patch: {...}, expectedUpdatedAt: "..." }` — сервер проверяет, что
  `updatedAt` совпадает с переданным, иначе `409` ("кто-то уже изменил
  эту запись, вот актуальная версия") вместо тихой перезаписи
- `PATCH /api/tasks/reorder` — отдельный batch-эндпоинт, меняет только
  порядок (`order`) у присланных `{id, order}` пар, без проверки
  `expectedUpdatedAt`; отвечает `{id, updatedAt}[]` по каждой
  переставленной записи (нужно клиенту, иначе следующий точечный
  `PATCH` словит ложный `409` — см. `LEARNING.md`, 2026-09-19/20)
- `DELETE /api/tasks/:id` — удалить одну запись
- На фронтенде это скрыто за паттерном `Repository<T>` +
  `usePersistedCollection` (`src/shared/lib/storage/`) — единый
  интерфейс `list/create/update/remove/reorder/refresh` с оптимистичным
  применением изменения локально и одним сетевым запросом на действие;
  сам компонент формы или провайдер не знает, что там REST, а не
  что-то другое
...
### 3.4 Что такое эти твои "агенты" в `.claude/`, и при чём тут `CLAUDE.md`

`/CLAUDE.md` (корень репозитория) — Claude Code читает его сам,
автоматически, в начале любой сессии в этой папке, на любом компьютере
и с любого аккаунта. Там — не "что за проект" (это здесь), а **как мы
договорились работать**: когда заходить в Plan Mode, что проверять
перед тем как считать фичу готовой, что документация — часть той же
правки, что пуш в `main` = деплой и требует подтверждения каждый раз.
**Если этот файл читаешь не в самом начале сессии — стоит вернуться и
прочитать `CLAUDE.md` тоже, если ещё не читал.**

Остальное в `.claude/` — не часть приложения, а инструкции для Claude
Code, специфичные под этот проект:
- `agents/project-guide` — отвечает на вопросы "почему это сделано так"
- `agents/qa-tester` — тестирует функциональность через настоящий браузер
- `agents/docs-writer` — пишет/обновляет документацию в
  `docs/requirements/` вместе с каждым изменением кода
- `skills/coding-mentor` — как объяснять код этому владельцу (полный
  новичок, учится по ходу дела)
...
`CLAUDE.md`) переносимо на любой AI, включая тебя, кто бы ты ни был.
...
```
TaskTrackerNew-main/
├── CLAUDE.md                  как мы работаем (Claude Code читает сам,
│                               автоматически, в начале любой сессии)
...
**Порядок чтения, если нужно разобраться с нуля**: `CLAUDE.md` (как
работаем) → этот файл → `docs/ARCHITECTURE.md` → `docs/FILE_GUIDE.md`
(если хочешь понять каждый конкретный файл) → `docs/DEPLOYMENT.md` →
конкретный файл в `docs/requirements/` под ту фичу, с которой
работаешь.
...
### 7.5 Потерян доступ к Claude / к этой переписке / сменился аккаунт

Ты уже это читаешь — значит, план сработал. Если открываешь проект
заново в Claude Code (на этом же компьютере или на новом) — `/CLAUDE.md`
в корне репозитория подхватится сам, автоматически, без необходимости
что-либо специально просить или напоминать про стиль работы. Дальше:
раздел 5 (файловая карта) → раздел 4 (как всё увидеть самому) → читай
`docs/ARCHITECTURE.md` и `docs/DEPLOYMENT.md` → всё остальное знание
там.

**Важно:** то, что ассистент запоминал сам по себе между сессиями
(автоматическая память Claude Code) — это локальный файл на *том*
компьютере/сессии, он не переедет вместе со сменой аккаунта или
компьютера. Всё, что действительно должно пережить потерю доступа,
уже вынесено сюда, в `CLAUDE.md`, `LEARNING.md` и `docs/` — то есть в
сам репозиторий. Если что-то важное всплывёт в разговоре с новой
сессией и не найдётся ни в одном из этих файлов — значит, это стоит
дописать в `LEARNING.md` или сюда прямо тогда же, а не полагаться, что
